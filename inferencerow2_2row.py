...
ROW_PT_PATH = "bestrackrow2.pt"     # model deteksi ROI gabungan row1 bawah + row2 tengah
BOX_PT_PATH = "bestkfslast.pt"      # model deteksi box

ROW_ONNX_PATH = "bestrackrow2.onnx"
BOX_ONNX_PATH = "bestkfslast.onnx"
...
# Model row mendeteksi satu ROI gabungan yang mencakup:
# - ROW2_TENGAH di bagian atas
# - ROW1_BAWAH di bagian bawah
ROW_SPECS = [
    {
        "label": "ROW1_BAWAH",
        "keywords": ("ROW1", "ROW 1", "R1", "BAWAH", "BOTTOM", "LOWER"),
    },
    {
        "label": "ROW2_TENGAH",
        "keywords": ("ROW2", "ROW 2", "R2", "TENGAH", "MIDDLE", "CENTER"),
    },
]
NUM_DETECTED_ROWS = len(ROW_SPECS)
GRID_ROWS = NUM_DETECTED_ROWS
GRID_COLS = 3

# Urutan posisi vertikal di gambar: atas=row2 tengah, bawah=row1 bawah.
GRID_POSITION_TO_ROW_INDEX = [1, 0]
...
# 2 = class sisanya
# FB / FR tetap diabaikan untuk row1/row2; hanya RB / RR yang dipakai.
# =========================================================
...
for cls_id, name in labels.items():
    ...
    if name_upper.startswith(VALID_ROW12_PREFIXES):
...
def format_single_row_output(row_output):
    return " ".join(str(int(value)) for value in row_output)


def format_rows_output(output):
    return " | ".join(
        f"{ROW_SPECS[row_idx]['label']}:{format_single_row_output(output[row_idx])}"
        for row_idx in range(NUM_DETECTED_ROWS)
    )
...
def make_empty_result(raw_output=None):
    if raw_output is None:
        raw_output = np.full((GRID_ROWS, GRID_COLS), EMPTY_VALUE, dtype=np.int32)
...
def build_grid_cells(row_bbox):
    x1, y1, x2, y2 = row_bbox
    x_edges = np.linspace(x1, x2, GRID_COLS + 1).astype(np.int32)
    y_edges = np.linspace(y1, y2, GRID_ROWS + 1).astype(np.int32)

    cells = np.empty((GRID_ROWS, GRID_COLS, 4), dtype=np.int32)

    for grid_row in range(GRID_ROWS):
        row_idx = GRID_POSITION_TO_ROW_INDEX[grid_row]
        for col in range(GRID_COLS):
            cells[row_idx, col] = np.array([
                x_edges[col],
                y_edges[grid_row],
                x_edges[col + 1],
                y_edges[grid_row + 1],
...
def best_row_from_result(result, frame_shape):
    ...
    cells = build_grid_cells(row_bbox)
...
def mark_cells_by_overlap(output, box_xyxy, box_values, box_cls, cells, frame):
    ...
    cells_flat = cells.reshape(-1, 4)

    for idx, (box, value, cls_id) in enumerate(zip(box_xyxy, box_values, box_cls)):
        bx1, by1, bx2, by2 = box
        box_area = max(1, int((bx2 - bx1) * (by2 - by1)))

        ix1 = np.maximum(bx1, cells_flat[:, 0])
        iy1 = np.maximum(by1, cells_flat[:, 1])
        ix2 = np.minimum(bx2, cells_flat[:, 2])
        iy2 = np.minimum(by2, cells_flat[:, 3])
        ...
        if inter_area[best_cell] > 0 and overlap_ratio >= MIN_BOX_CELL_OVERLAP_RATIO:
            ...
            hsv_passed[idx] = True
            row_idx = best_cell // GRID_COLS
            col_idx = best_cell % GRID_COLS
            output[row_idx, col_idx] = int(value)
...
def run_inference(frame, row_model, box_model, state, frame_id):
    ...
    Output result dict:
    {
        ...
        cells,
        boxes,
        raw_output: matrix 2x3 sebelum temporal smoothing,
        output: matrix 2x3
    }
    """
    ...
    if not state.has_cached_row(frame_id):
        output = np.full((GRID_ROWS, GRID_COLS), EMPTY_VALUE, dtype=np.int32)
...
def render_frame(frame, result):
    ...
    if not result["detected"] or result["row_bbox"] is None:
        ...
        cv2.putText(
            frame,
            f"ROW12 OUTPUT: {format_rows_output(output)}",
            (20, h - 55),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.65,
            (0, 255, 255),
    ...
    x1, y1, x2, y2 = row_bbox
    row_label = "ROW12 ROI CACHE" if result.get("row_from_cache") else "ROW12 ROI"
    ...
    for row_idx in range(GRID_ROWS):
        for col_idx in range(GRID_COLS):
            cx1, cy1, cx2, cy2 = cells[row_idx, col_idx]
            cv2.rectangle(frame, (cx1, cy1), (cx2, cy2), (0, 255, 255), 2)

            cell_value = int(output[row_idx, col_idx])
            color = CELL_VALUE_COLORS.get(cell_value, (0, 255, 255))
            row_text_label = ROW_SPECS[row_idx]["label"]
...
def main():
    ...
    print("======================================")
    print("ROW1_BAWAH + ROW2_TENGAH + BOX OPTIMIZED INFERENCE")
    print("======================================")
    ...
    print(
        "ROWS           : "
        + ", ".join(spec["label"] for spec in ROW_SPECS)
    )
    ...
    try:
        while running:
            ...
            if result is not None and now - last_print > 0.5:
                output = result["output"]
                print(
                    f"ROW12: {format_rows_output(output)} | "
                    f"{profiler.text()}"
                )
                last_print = now

            if SHOW_WINDOW:
                cv2.imshow("Improved ROW1/ROW2 Rack + Box Detection", rendered)
